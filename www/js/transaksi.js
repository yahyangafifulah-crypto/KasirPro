...
function muatBarang() {
    ...
    daftarBarang.forEach((barang, index) => {
        ...
        pilih.innerHTML += `
        <option value="${index}">
            ${barang.nama} - Rp ${barang.hargaJual}
        </option>
        `;
...
function tambahKeranjang() {
    ...
    let item = {
        ...
        harga: parseInt(daftarBarang[index].hargaJual),
        jumlah: jumlah,
        subtotal: parseInt(daftarBarang[index].hargaJual) * jumlah
    };
...
keranjang.forEach(function(item){
        ...
        daftarBarang[item.index].stok =
            parseInt(daftarBarang[item.index].stok) - item.jumlah;

    });

    localStorage.setItem(
        "barang",
        JSON.stringify(daftarBarang)
    );
